...
function http_request(args) {
    return new Promise((resolve, reject) => {
        ...
        const req = transport.request(req_args, res => {
            let resp_body = ''
            res.on('data', data => (resp_body += data))
            res.on('error', err => reject(err))
            res.on('end', () => {
                if (args.expected_status_code && res.statusCode != args.expected_status_code) {
                    const error_message = `HTTP request did not return (${res.statusCode}) the expected status code (${args.expected_status_code}): "${resp_body.toString()}"`
                    log(error_message)
                    return reject(error_message)
                }
                const resolver = Object.assign({ statusCode: res.statusCode, body: resp_body }, res.headers)
                resolve(resolver)
            })
        })
        if (args.headers) {
        ...
        if (body) {
            log('Sending the body as %s', JSON.stringify(body).substring(0, 300))
