...
const debug = Debug('stocto:lib:stocto')
const fs = require('fs')
...
class Stocto {
    ...
    async do_analysis(position) {
        ...
        if (this.is_it_checkmate(out.info)) {
            analysis.score = position.fen.split()[ 1 ] == 'w' ? 1000 : -1000
        }
        else {
    ...
    is_it_checkmate(info) {
        return info
            .find(record => record.score && record.score.unit && record.score.unit == 'mate')
    }
    ...
    async do_benchmark() {
        ...
        const log = debug.extend('benchmark')
        console.log('Doing benchmark')

        await this.uci.init()
        console.log('done init()')
        await this.uci.ucinewgame()
        console.log('done ucinewgame()')

        this.uci.proc.stdout.on('data', data => log(data))
        console.log('done setting action')
        await this.uci.sendCmd(`bench`)
        console.log('done sending bench')
        await this.uci.isready()
    }
}
